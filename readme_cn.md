...
10. **0.4.1版本新增**：本次版本重点围绕加密通信与代理转发进行升级：

    10.1 **ChaCha20 加密shell**：反向/绑定shell的所有 I/O 流量使用 ChaCha20 流密码进行端到端加密。密钥派生方式为 口令 → SHA-256 → 256-bit 密钥，并使用两个独立的 nonce 分别加密 server→agent 与 agent→server 两个方向的流量。底层采用 pipe+fork 的中继模式实现，已覆盖 6 种架构，生成的二进制文件大小约 3–7KB。

    10.2 **SOCKS5 代理二进制**：纯汇编编写的 ELF 文件（约 2–5KB），支持反向和正向 SOCKS5 代理隧道，可选启用 RFC 1929 用户名/口令认证，方便在嵌入式设备上进行轻量级内网穿透与横向移动。

    10.3 **完整 PTY 交互终端模式**：反弹/绑定shell 可以运行在真正的伪终端下，支持作业控制、行编辑以及 vim/top 等 curses 类交互程序。
...
2026.4.7 0.4.1 新增基于 ChaCha20 的端到端加密反向/绑定 shell（口令 → SHA-256 → 256-bit 密钥，双向独立 nonce，支持 6 种架构，生成文件约 3–7KB）；新增纯汇编 SOCKS5 代理 ELF（约 2–5KB），支持反向/正向隧道与 RFC 1929 认证；新增完整 PTY 交互终端模式；新增带 tab 补全与历史命令的多 shell 会话管理器；新增统一服务端模式，可在单一控制台中同时管理 SOCKS5 代理与多会话 shell
