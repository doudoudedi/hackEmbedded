...
# Legacy path constant - kept for backwards compatibility check during migration
...
def data_base_init():
	"""
	Initialize the model_tree with device data.
	If database already has data, load from database.
	Otherwise, initialize with hardcoded data and save to database.
	"""
	global model_tree, model_arch_tree
	...
	# Initialize with hardcoded data (first run)
	...
	model_tree["DrayTek_Vigor2960"] = ["armelv7", "router", "linux", "Mindspeed", "Mindspeed Comcerto 2000(Cortex-A9)", "lighttpd", "False", "False", "root", "NULL", "unknow", "False", "True", exp_database.model_exp_dic["DrayTek_Vigor2960"]]
	...
	model_tree["Netgear_RAX30"] = ["armelv7 && aarch64", "router", "linux4.19.0", "Broadcom", "BCM4908(Cortex-A53和Cortex-A57)", "lighttpd", "False", "False","admin", "Default passwd", "unknow", "True", "True", exp_database.model_exp_dic['Netgear_RAX30']]
	...
	# Save to database on first initialization
	_save_to_database()


def _load_from_database():
	"""Load device data from database into model_tree and model_arch_tree."""
	global model_tree, model_arch_tree

	# Get all devices from database
	devices = database.get_all_devices()
	all_exploits = database.get_all_exploits()

	for device in devices:
		model_name = device['model_name']
		info_list = database.device_to_info_list(device)
		# Add exploits as the 14th element
		exploits = all_exploits.get(model_name, {})
		info_list.append(exploits)
		model_tree[model_name] = info_list
		model_arch_tree[model_name] = info_list[0]

	# Load arch cache
	arch_cache = database.get_all_arch_cache()
	for model_name, arch in arch_cache.items():
		if model_name not in model_arch_tree:
			model_arch_tree[model_name] = arch


def _save_to_database():
	"""Save model_tree data to database."""
	for model_name, info_list in model_tree.items():
		# Save device info (first 13 elements)
		database.insert_device(model_name, info_list[:13])

		# Save exploits if present (14th element is the exploit dict)
		if len(info_list) > 13 and info_list[13] and isinstance(info_list[13], dict):
			for cve_id, exploit_data in info_list[13].items():
				if isinstance(exploit_data, list) and len(exploit_data) >= 2:
					database.insert_exploit(model_name, cve_id, exploit_data[0], exploit_data[1])
				elif isinstance(exploit_data, str):
					database.insert_exploit(model_name, cve_id, '', exploit_data)

	# Save arch cache
	for model_name, arch in model_arch_tree.items():
		database.update_arch_cache(model_name, arch)
...
def dict_to_txt(dic1):
	"""Save model-arch mapping to database (replaces file-based storage)."""
	try:
		for k, v in dic1.items():
			database.update_arch_cache(str(k), str(v))
	except Exception as e:
		pass
		#log.success("error "+ e)
...
def txt_to_dict():
	"""Load model-arch mapping from database (replaces file-based storage)."""
	global model_arch_tree
	try:
		arch_cache = database.get_all_arch_cache()
		for k, v in arch_cache.items():
...
def append_to_tree(model, arch):
	"""Add or update a model-arch mapping."""
	global model_arch_tree
	txt_to_dict()
	model_arch_tree[model] = arch
	database.update_arch_cache(model, arch)

def model_to_arch(model):
	"""Get architecture for a given model."""
	# First try from memory
	if model in model_arch_tree:
		return model_arch_tree[model]
	# Then try from database cache
	arch = database.get_arch_cache(model)
	if arch:
		model_arch_tree[model] = arch
		return arch
	# Finally try from model_tree
	if model in model_tree:
		return model_tree[model][0]
...
def model_tree_dic():
	"""Save model_tree data to database (replaces file-based storage)."""
	for k, v in model_tree.items():
		# Save device info (first 13 elements)
		info_list = v[:13] if len(v) >= 13 else v
		database.insert_device(k, info_list)

		# Save exploits if present (14th element is the exploit dict)
		info_len = len(v)
		if info_len > 13 and v[13] and isinstance(v[13], dict):
			for cve_id, exploit_data in v[13].items():
				if isinstance(exploit_data, list) and len(exploit_data) >= 2:
					database.insert_exploit(k, cve_id, exploit_data[0], exploit_data[1])
				elif isinstance(exploit_data, str):
					database.insert_exploit(k, cve_id, '', exploit_data)


def check_poc_info(path, model, vul):
	"""Check POC info - returns description from model_tree or database."""
	try:
		# First try from model_tree in memory
		if model in model_tree and len(model_tree[model]) > 13:
			exploits = model_tree[model][13]
			if isinstance(exploits, dict) and vul in exploits:
				return exploits[vul][0]
		# Fall back to database
		exploits = database.get_exploits(model)
		if vul in exploits:
			return exploits[vul][0]
		return ''
	except:
...
def dic_model_tree():
	"""Load model_tree data from database (replaces file-based storage)."""
	global model_tree, model_arch_tree
	...
	for device in devices:
		model_name = device['model_name']
		# Skip if already in model_tree (from hardcoded init)
		if model_name in model_tree:
			continue

		model_info = database.device_to_info_list(device)
		# Add exploits as the 14th element
		exploits = all_exploits.get(model_name, {})
		model_info.append(exploits)
		model_tree[model_name] = model_info
		model_arch_tree[model_name] = model_info[0]
...
def list_model_tree():
	...
	print("-" * 0x42)
	log.success("The total number of storage devices in the data is {}".format(str(database.get_device_count())))

	log.success("The number of POCs corresponding to the model is {}".format(str(database.get_exploit_count())))
...
def add_model_info():
	...
	model_arch_tree[model_add] = model_info_list[0]

	# Save to database
	database.insert_device(model_add, model_info_list[:13])
	database.update_arch_cache(model_add, model_info_list[0])

	# Save exploits if present
	if len(model_info_list) > 13 and model_info_list[13] and isinstance(model_info_list[13], dict):
		for cve_id, exploit_data in model_info_list[13].items():
			if isinstance(exploit_data, list) and len(exploit_data) >= 2:
				database.insert_exploit(model_add, cve_id, exploit_data[0], exploit_data[1])
			elif isinstance(exploit_data, str):
				database.insert_exploit(model_add, cve_id, '', exploit_data)
...
def search_CVE(CVE_ID):
	"""Search for CVE information in model_tree and database."""
	flag = 0
	value_pattern = re.compile(CVE_ID, re.IGNORECASE)

	# Search in memory (model_tree)
	for value in model_tree.values():
		if len(value) > 13 and isinstance(value[13], dict):
	...
	all_exploits = database.get_all_exploits()
	for model_name, exploits in all_exploits.items():
		for cve_id, exploit_data in exploits.items():
			if value_pattern.match(cve_id) and flag == 0:
				print(Fore.GREEN + cve_id + Fore.RESET + "  :  " + exploit_data[0])
				print("-" * 0x40)
				print("EXP or POC: ")
				print(exploit_data[1] + Fore.RESET)
				print("-" * 0x40)
				flag = 1
